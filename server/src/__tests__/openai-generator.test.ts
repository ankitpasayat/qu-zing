...
import { generateQuestionsWithOpenAI } from '../lib/openai-question-generator.js';
...
describe('OpenAI Question Generator', () => {
  ...
  it('should throw error when API key is not set', async () => {
    ...
    await expect(generateQuestionsWithOpenAI({ count: 5 }))
  ...
  it('should throw AbortError when signal is already aborted', async () => {
    ...
    await expect(generateQuestionsWithOpenAI({ 
      count: 5, 
  ...
  it('should use gpt-4o-mini model', async () => {
    ...
    await generateQuestionsWithOpenAI({ count: 1 });
  ...
  it('should use categories when provided', async () => {
    ...
    const callBody = JSON.parse((mockFetch.mock.calls[0][1] as RequestInit).body as string);
    expect(callBody.messages[1].content).toContain('Science, History');
  });
  ...
  it('should use difficulties when provided', async () => {
    ...
    const callBody = JSON.parse((mockFetch.mock.calls[0][1] as RequestInit).body as string);
    expect(callBody.messages[1].content).toContain('hard');
  });
});
